...
function drawGradients(ctx: CanvasRenderingContext2D, width: number, height: number, colors: string[], energies?: number[], image?: HTMLImageElement) {
  ...
  const gradient = ctx.createRadialGradient(width, height, 0, width, height, r1);
  ctx.globalAlpha = 0.9;
  if (energies && energies.length === 3 && energies.every(energy => energy > 0)) {
...
export const shuffle = (columns) => {
  ...
  const cells = Array.from({ length: totalCells }, (_, i) => totalCells - (i + 1));
  return cells;
}



function drawShuffledGradients(ctx: CanvasRenderingContext2D, width: number, height: number, imageData: HTMLCanvasElement, columns: number) {
  ...
  for (let i = 0; i < totalCells; i++) {
    ...
    if (columns > 1) {
      ctx.setTransform(
        true ? -1 : 1, 0, // set the direction of x axis
        0, true ? -1 : 1,   // set the direction of y axis
    ...
    ctx.drawImage(imageData, sx * cellWidth, sy * cellHeight, cellWidth, cellHeight, dx, dy, destCellWidth, destCellHeight);
...
export default function Hero({ name }: { name: string }) {
  ...
  const secondary = useRef(toColorString(convert.hex.hsl(currentPalette.palette.properties.secondary)))
  const tertiary = useRef(toColorString(convert.hex.hsl(currentPalette.palette.properties.tertiary)))
      ...
      if (motionAnalyzerRef.current) {
        ...
        motionAnalyzerRef.current = new AudioMotionAnalyzer(undefined, {
          ...
          onCanvasDraw: (instance) => {
            ...
            const drawFunction = (ctx: CanvasRenderingContext2D) => drawGradients(ctx, width, height, colors, energies, imageRef.current)
            const gradientImageData = getImageDataFromDrawingFunction(width, height, drawFunction);
            if (!gradientImageData) return console.error('no gradient image data')
            drawShuffledGradients(ctx, width, height, gradientImageData, columns.current);
  ...
  useEffectOnce(() => {
    onUpdate.current = (latest) => {
      ...
      const drawingFunction = (ctx: CanvasRenderingContext2D) => drawGradients(ctx, canvas.width, canvas.height, colors, energies, imageRef.current)
      const gradientImageData = getImageDataFromDrawingFunction(canvas.width, canvas.height, drawingFunction);
      if (!gradientImageData) return console.error('no gradient image data')
      drawShuffledGradients(ctx, canvas.width, canvas.height, gradientImageData, columns.current);
  ...
  useUpdateEffect(() => {
    ...
    if (!ctx) return console.error('no context')
    const colors = [primary.current, secondary.current, tertiary.current]
    const drawingFunction = (ctx: CanvasRenderingContext2D) => drawGradients(ctx, width, height, colors, undefined, imageRef.current)
    const gradientImageData = getImageDataFromDrawingFunction(width, height, drawingFunction);
    if (!gradientImageData) return console.error('no gradient image data')
    drawShuffledGradients(ctx, width, height, gradientImageData, columns.current);
